import { getAllCities } from "@/services/address-service";
...
export default function City() {
  ...
  return (
    <>
      ...
      <div className="mx-auto text-center text-3xl mt-8 mb-6 font-semibold">Kabupaten/Kota</div>
      <div className="container mx-auto grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 mb-8 px-4 sm:px-0">
        {isLoading ? (
          ...
          data?.result.map((item: Address, index: number) => (
            <Card key={item.id} className="cursor-pointer" onClick={() => navigate(`/address/kecamatan/${item.id}`)}>
              <CardHeader>
